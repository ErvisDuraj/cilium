...
* The tc BPF programs do not require any driver changes since they are run
  ...
  **after** traditional iptables firewalling hooks like iptables POSTROUTING,
  but still before handing the packet to the kernel's GSO engine.
...
* **Forwarding and load-balancing**
  ...
  to the container itself. Egress traffic is due to the nature of the kernel's
  networking stack already ``skb`` based anyway such that packet rewrites and
  redirects are suitable out of tc BPF. Through utilizing the ``bpf_redirect()``
  helper function, BPF can take over the forwarding logic to push the ``skb``
  either into the ingress or egress path of another networking device. Thus,
  any bridge-like devices become unnecessary to use as well by utilizing
  tc BPF as forwarding fabric.
